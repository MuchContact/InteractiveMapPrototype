...
/**
 * Class: OpenLayers.Tile.TFS
 * Instances of OpenLayers.Tile.TFS are used to manage 
 * TFSs. This is an unusual tile type in that it doesn't have a
 * rendered image; only a 'hit grid' that can be used to 
 * look up feature attributes.
 *
 * See the <OpenLayers.Tile.TFS> constructor for details on constructing a
 * new instance.
 *
 * Inherits from:
 *  - <OpenLayers.Tile>
 */
OpenLayers.Tile.TFS = OpenLayers.Class(OpenLayers.Tile, {

    /** 
     * Property: url
     * {String}
     * The URL of the TFS file being requested. Provided by the <getURL>
     *     method. 
     */
    url: null,
    ...
    /** 
     * Constructor: OpenLayers.Tile.TFS
     * Constructor for a new <OpenLayers.Tile.TFS> instance.
     * 
     * Parameters:
     ...
     * In the case of TFSs, "drawing" it means fetching and
     * parsing the json. 
     * 
     * Returns:
    ...
    getFeatureInfo: function(i, j) {
        ...
        if (this.json) {
			...
			var that = this;
			_.each(this.json, function(item){
				if(that.inBox(item.geometry, i, j)){
					results.push(item);
    ...
    inBox: function(geometry, i, j) {
		var pixel = this.layer.map.getPixelFromLonLat(new OpenLayers.LonLat(geometry.x, geometry.y));
		var newI = this.position.x + this.layer.map.layerContainerOriginPx.x + i;
		var newJ = this.position.y + this.layer.map.layerContainerOriginPx.y + j;
		//console.log(this.position+'-'+i+'-'+j);
		if(newI <= pixel.x+this.utfgridResolution
			&& newI >= pixel.x-this.utfgridResolution
			&& newJ <= pixel.y+this.utfgridResolution
			&& newJ >= pixel.y-this.utfgridResolution){
			return true;
		}
		return false;
     ...
     * Given a character code for one of the TFS "grid" characters, 
     *     resolve the integer index for the feature id in the TFS "keys"
     *     array.
     *
     * Parameters:
     * charCode - {Integer}
     ...
     *
     * Parameters:
     * str - {String} TFS as a JSON string. 
     * 
     * Returns:
    ...
    CLASS_NAME: "OpenLayers.Tile.TFS"
